...
class TranslatorAgent:
    ...
    def _translate_text(
        ...
        if not text:
            logger.debug("Input text to _translate_text is empty. Returning empty string.")
            return ""
        ...
        try:
            ...
            if isinstance(response_text, str):
                if response_text == "" and processed_text != "": # LLM returned empty string for non-empty input
                    logger.info("LLM returned an empty string. Assuming it's an intentional empty translation.")
                    translated_text_with_placeholders = "" # Use empty string for re-insertion
    ...
    def translate_input_to_english(self, message: str) -> str:
        ...
        if self.source_language != "en":
            logger.info(
                f"Translating input from {self.source_language} to English."
            )
    ...
    def translate_output_to_source_language(self, message: str) -> str:
        ...
        if self.source_language and self.source_language != "en":
            logger.info(
                f"Translating output from English to {self.source_language}."
            )
            logger.info(
