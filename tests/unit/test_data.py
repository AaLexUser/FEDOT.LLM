...
class TestDataset:
    ...
    def test_dataset_preview_train_lte_10_cols(self, mock_get_train):
        ...
        with patch.object(train_df, 'sample') as mock_train_sample, \
            ...
            assert "Columns:" not in preview # Should show samples, not just columns

    def test_dataset_preview_no_train_split(self): # Removed mock_get_train
        # This scenario implies get_train_split would raise an error or return None
        # Based on current get_train_split, it raises ValueError if no splits.
        # Let's test the case where get_train_split fails by making splits empty.
        dataset_no_splits = Dataset(splits=[], path=MagicMock(spec=Path)) # Use spec for Path mock
        with pytest.raises(ValueError): # Propagated from get_train_split
