...
```"""
    ...
    def test_reinsert_code_blocks_placeholder_missing_from_map(self, mock_inference_fixture):
        agent = TranslatorAgent(inference=mock_inference_fixture)
        placeholder = f"{agent.code_block_placeholder_prefix}_0__"
    ...
    def test_reinsert_code_blocks_placeholder_not_in_text(self, mock_log_warning, mock_inference_fixture):
        agent = TranslatorAgent(inference=mock_inference_fixture)
        ...
        assert mock_log_warning.call_count == 3 # Two for individual placeholders, one summary
    ...
    def test_translate_text_source_language_none(self, mock_extract, mock_reinsert, mock_inference_fixture):
        agent = TranslatorAgent(inference=mock_inference_fixture)
        text = "Some text"
        ...
        assert "Translate the following text from the auto-detected source language to en." in prompt
    ...
    @patch.object(TranslatorAgent, '_reinsert_code_blocks')
    @patch.object(TranslatorAgent, '_extract_code_blocks')
    def test_translate_text_source_language_en(self, mock_extract, mock_reinsert, mock_inference_fixture):
        agent = TranslatorAgent(inference=mock_inference_fixture)
        text = "Some text"
    ...
    @patch('fedotllm.agents.translator.logger.warning')
    @patch.object(TranslatorAgent, '_extract_code_blocks')
    def test_translate_text_llm_returns_none(self, mock_extract, mock_log_warning, mock_inference_fixture):
        agent = TranslatorAgent(inference=mock_inference_fixture)
        ...
        mock_log_warning.assert_called_once_with(
            f"LLM query did not return a valid string (type: <class 'NoneType'>, value: 'None...'). Returning original text."
        )
        mock_extract.assert_called_once_with(text)
    ...
    @patch('fedotllm.agents.translator.logger.warning')
    @patch.object(TranslatorAgent, '_extract_code_blocks')
    def test_translate_text_llm_returns_non_string(self, mock_extract, mock_log_warning, mock_inference_fixture):
        agent = TranslatorAgent(inference=mock_inference_fixture)
        text = "Some text"
        processed_text = "Processed text" # This won't actually be used if LLM fails early
        mock_extract.return_value = (processed_text, {}) # Mocked but _translate_text might return before using its output fully
        ...
        mock_log_warning.assert_called_once_with(
            f"LLM query did not return a valid string (type: <class 'int'>, value: '123...'). Returning original text."
        )
        mock_extract.assert_called_once_with(text) # extract is still called before the query

    @patch.object(TranslatorAgent, '_reinsert_code_blocks')
    @patch.object(TranslatorAgent, '_extract_code_blocks')
    def test_translate_text_processed_equals_original(self, mock_extract, mock_reinsert, mock_inference_fixture):
        agent = TranslatorAgent(inference=mock_inference_fixture)
        text = "Text without code blocks."
        llm_response = "Translated text without code blocks."

        mock_extract.return_value = (text, {}) # processed_text is the same as text, empty code_map
        mock_inference_fixture.query.return_value = llm_response
        mock_reinsert.return_value = llm_response # reinsert will just return llm_response
    ...
    @patch.object(TranslatorAgent, '_reinsert_code_blocks')
    @patch.object(TranslatorAgent, '_extract_code_blocks')
    def test_translate_text_llm_returns_same_as_processed(self, mock_extract, mock_reinsert, mock_inference_fixture):
        agent = TranslatorAgent(inference=mock_inference_fixture)
        text = "Text with ```code```"
        ...
        mock_reinsert.side_effect = lambda txt, cmap: txt.replace("placeholder", cmap["placeholder"])
    ...
    @patch('fedotllm.agents.translator.detect')
    def test_translate_input_to_english_empty_message(self, mock_detect, mock_inference_fixture):
        agent = TranslatorAgent(inference=mock_inference_fixture)
    ...
    def test_translate_output_to_source_language_empty_message(self, mock_translate_text, mock_inference_fixture):
        agent = TranslatorAgent(inference=mock_inference_fixture)
    ...
    @patch.object(TranslatorAgent, '_translate_text')
    def test_translate_output_to_source_language_no_source_lang(self, mock_translate_text, mock_inference_fixture):
        agent = TranslatorAgent(inference=mock_inference_fixture)
        ...
        assert translated_output == message # Should return original if source_lang is None
        mock_translate_text.assert_not_called()

    @patch.object(TranslatorAgent, '_translate_text')
    def test_translate_output_to_source_language_is_english(self, mock_translate_text, mock_inference_fixture):
        agent = TranslatorAgent(inference=mock_inference_fixture)
        agent.source_language = 'en' # Source language was English
        message = "Some output message"
        translated_output = agent.translate_output_to_source_language(message)
        assert translated_output == message # Should return original if source_lang is 'en'
        mock_translate_text.assert_not_called()
