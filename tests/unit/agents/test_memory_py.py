...
import chromadb # For spec in mocks
# from chromadb.api.client import Client # Not strictly needed if using spec with string
# from chromadb.api.types import QueryResult # Not strictly needed if using spec with string
...
def test_memory_init(mock_chroma_client, mock_embedding_model, mock_chroma_collection):
    ...
    # Correcting the assertion to match positional if that's how it's called,
    # or ensuring keyword usage if preferred. The Memory __init__ is (client, collection_name, embedding_model)
    # The test calls it with keyword args, which is fine.
    # The mock assertion was failing because the order of keyword args in expected vs actual string representation.
    # Using positional in assert_called_once_with for simplicity if order is fixed.
    # Or, ensure the Memory __init__ defines them as keyword-only if that's intended.
    # For now, assume the keywords are fine but the mock framework might be picky on string format of call.
    # The actual call in Memory is: self.chroma_client.get_or_create_collection(self.collection_name, metadata=...)
    mock_chroma_client.get_or_create_collection.assert_called_once_with(
        collection_name, # name is positional
...
def test_memory_insert_vectors(mock_chroma_client, mock_embedding_model, mock_chroma_collection, mocker):
    ...
    # Mock the return value of embedding_model.encode
    # Assuming encode returns a list of objects, each having an 'embedding' attribute
...
def test_memory_search_context(mock_chroma_client, mock_embedding_model, mock_chroma_collection):
    ...
    mock_chroma_collection.query.assert_called_once_with(
        ...
        # where=None is not explicitly passed by the code, so not asserted here
    )
...
def test_memory_search_context_with_metadatas(mock_chroma_client, mock_embedding_model, mock_chroma_collection):
    ...
    mock_embedding_model.encode.assert_called_once_with(query)
    mock_chroma_collection.query.assert_called_once_with(
        query_embeddings=[[0.3, 0.4, 0.5]], # Correctly wrapped
        n_results=n_results,
...
def test_memory_check_collection_none_empty(mock_chroma_client, mock_embedding_model, mock_chroma_collection):
    ...
    count = memory.check_collection_none() # Corrected method name
    assert count == 0
