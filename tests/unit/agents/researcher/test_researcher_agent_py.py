...
# Using a class to group tests and patches for ResearcherAgent
@patch('fedotllm.agents.researcher.researcher.is_useful') # Assuming this is the correct path
@patch('fedotllm.agents.researcher.researcher.is_grounded') # Assuming this is the correct path
@patch('fedotllm.agents.researcher.researcher.is_continue') # Assuming this is the correct path
...
@patch('fedotllm.agents.researcher.researcher.END', new_callable=lambda: "END_NODE_RESEARCHER")
@patch('fedotllm.agents.researcher.researcher.START', new_callable=lambda: "START_NODE_RESEARCHER")
@patch('fedotllm.agents.researcher.researcher.StateGraph')
class TestResearcherAgent:

    def test_researcher_agent_init(self, mock_sg_ignored, mock_start_ignored, mock_end_ignored,
                                   mock_rd_ignored, mock_gr_ignored, mock_gen_resp_ignored,
                                   mock_ra_ignored, mock_rq_ignored, mock_ic_ignored,
                                   mock_ig_ignored, mock_iu_ignored,
                                   mock_inference, mock_embeddings):
        # All patches are active but not directly checked in __init__
    ...
    def test_researcher_agent_create_graph(self, mock_state_graph_constructor,
                                           mock_start_node_str, mock_end_node_str,
                                           ...
                                           mock_is_continue_func, # These are now patched functions
        ...
        mock_workflow_instance.add_edge.assert_any_call("render_answer", mock_end_node_str)
        ...
        assert conditional_edge_calls[0][0][0] == "retrieve_grader"  # source node
        assert callable(conditional_edge_calls[0][0][1])  # condition is a callable (lambda)
        assert conditional_edge_calls[0][0][2] == {True: "generate", False: "rewrite_question"}  # path_map

        # Second conditional edge (for "generate")
        assert conditional_edge_calls[1][0][0] == "generate"  # source node
        assert callable(conditional_edge_calls[1][0][1])  # condition is a callable (lambda)
        assert conditional_edge_calls[1][0][2] == {True: "render_answer", False: "generate"}  # path_map

        mock_workflow_instance.compile.assert_called_once()
        mock_compiled_workflow.with_config.assert_called_once_with(config={"run_name": "ResearcherAgent"})
